				     ...
				     unsigned int alg, unsigned int n, unsigned int l)
{
	...
	while (n-- > 0) {
		...
		res = TEEC_InvokeCommand(&sess, TA_ACIPHER_PERF_CMD_SIGN,
					 &op, &ret_origin);
		if (res)
			return res;
				       ...
				       unsigned int alg, unsigned int n, unsigned int l)
{
	...
	while (n-- > 0) {
		...
		res = TEEC_InvokeCommand(&sess, TA_ACIPHER_PERF_CMD_VERIFY,
					 &op, &ret_origin);
		if (res)
			return res;
				   ...
				   unsigned int alg, unsigned int n, unsigned int l)
{
	...
	case TA_ALG_SM2_DSA_SM3:
		ret = acipher_perf_sign(key_alg, key_size, alg, n, l);
		if (ret)
			return ret;

		ret = acipher_perf_verify(key_alg, key_size, alg, n, l);
		if (ret)
			return ret;
		break;
